...
/* These are for interfacing with time services and are not normally needed in a
 * sketch */
...
time_t makeTime(const tmElements_t &tm) {
    ...
    // note year argument is offset from 1970 (see macros in time.h to convert
    // to other formats) previous version used full four digit year (or digits
    // since 2000),i.e. 2009 was 2009 or 9
...
void setTime(time_t t) {
    ...
    if (sysUnsyncedTime == 0)
        sysUnsyncedTime =
            t;  // store the time of the first call to set a valid Time
#endif
    ...
    Status       = timeSet;
    prevMillis =
        millis();  // restart counting from now (thanks to Korman for this fix)
}
